...
jQuery(document).ready(function() {
	...
	$("#videoLoadmore").click(function(event) {
		...
		$(".additional-video-section:hidden").first().show("slow", function() {});
	});
	...
	// Working through Accesibilty Issues - CBC
	$(".header .dropdown-toggle").on("mouseover focus", function(event) {
		if (jQuery(window).width() > 1055) {
			...
			//console.log('Xoffset'+xOffset +'something'+something);
			if (xOffset > 700) {
	...
	if (jQuery(window).width() < 1055) {
		...
		$(".header .dropdown-toggle").on("click", function(event) {
			event.preventDefault();
	...
	if (jQuery("#object1").length > 0) {
		...
		jQuery(function() {
			jQuery(window).scroll(function() {
				...
				var object_position_top1 = window_height1 * (scroll_position1 / document_height1) + 120;
				jQuery('#object1').css({
	...
	jQuery('.content-carousel-slide').slick({
		slidesToShow: 1,
		slidesToScroll: 1,
		infinite: true,
		arrows: true,
	...
	$('.stacked-slider').on('afterChange', function(event, slick, direction) {
	...
	// Faculty Landing - Responsiveness Btn functionality 
	$("#facultyLoadmore").click(function(event) {
		event.preventDefault();
		$(".faculty-row").addClass('load-more-touched');
		$(".faculty-listing-item:hidden").slice(0, 4).show();
	});
	...
	$("#section-nav-ul .dropdown-toggle").on('click', function() {
...
jQuery(document).ready(function() {
	...
	jQuery(window).scroll(function(event) {
		...
		if ($(this).scrollTop() > realHeaderheight) {
...
jQuery(document).ready(function() {
		...
		$('.slider-col').on("focus", function() {
			var ele = $(this);
			var eleWidth = $(this).width();
			$('.slider').animate({
				scrollLeft: ele.offset().left + 'px'
			}, 'fast');
		});
	...
	var slidercolWidth = $('.slider-col').width();

	if (buttonRight) {
		buttonRight.onclick = function () {
			document.getElementById('sliderPost').scrollLeft += slidercolWidth;
		  };
	}
